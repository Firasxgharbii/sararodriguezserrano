...
function cleanGalleryImages(images: unknown[]): GalleryImageItem[] {
  return images
...
function cleanContentBeforeSave(content: SiteContent): SiteContent {
  return {
    ...content,
    oeuvres: {
      ...content.oeuvres,
      items: content.oeuvres.items.map((item) => ({
        ...item,
        galleryImages: cleanGalleryImages((item.galleryImages ?? []) as unknown[]),
      })),
    },
  };
}

function normalizeContent(parsed: any): SiteContent {
  return {
    ...defaultSiteContent,
    ...parsed,
    ...
    oeuvres: {
      ...
      items: (
        ...
        const defaultItem =
          (defaultSiteContent.oeuvres.items[index] as Partial<OeuvreItem>) ??
          {};
        ...
        return {
          ...
          imageSize: (item?.imageSize ??
            defaultItem?.imageSize ??
            "medium") as ImageSize,

          titleSize: (item?.titleSize ??
            defaultItem?.titleSize ??
            "large") as TitleSize,
...
export default function DashboardPage() {
  ...
  const saveContent = async () => {
    try {
      const cleanContent = cleanContentBeforeSave(content);
                  ...
                  <div className="mb-6 flex flex-col gap-4 rounded-[24px] border border-[#ece3dc] bg-[#fcfaf8] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-6 text-[#8a7971]">
                        {content.oeuvres.items.length} / {MAX_OEUVRES} œuvres
                        ajoutées.
                      </p>
                    </div>
                      ...
                      >
                        ...
                        <div className="grid gap-5">
                          ...
                          <SelectField
                            ...
                            onChange={(value) => {
                              const updated = [...content.oeuvres.items];
                              updated[index].imageSize = value as ImageSize;
                          ...
                          <SelectField
                            ...
                            value={item.titleSize ?? "large"}
                            onChange={(value) => {
                              const updated = [...content.oeuvres.items];
                              updated[index].titleSize = value as TitleSize;
                            ...
                            <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (
                                    currentImages.length >= MAX_GALLERY_IMAGES
                                  ) {
                                    return;
                                  }
                            ...
                            {(item.galleryImages ?? []).length > 0 ? (
                              <div className="grid gap-5 sm:grid-cols-2">
                                {(item.galleryImages ?? []).map(
                                  (galleryImage: any, imageIndex: number) => {
                                    const imageSrc =
                                      typeof galleryImage === "string"
                                        ? galleryImage
                                        : galleryImage?.src ?? "";
                              ...
                              <div className="rounded-[20px] border border-dashed border-[#ddd1ca] bg-[#fcfaf8] px-5 py-10 text-center">
                                ...
                                <p className="mt-2 text-sm text-[#8a7971]">
                                  Cliquez sur “Ajouter une image” pour
                                  commencer.
                                </p>
                              </div>
                            )}
